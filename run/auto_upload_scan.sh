...
find ~/Desktop/automation/APKs -type f -exec md5sum {} + | sort -k 2 > ~/Desktop/automation/scans_comparison/samples.txt

# Create a txt file which contains all the samples (md5 hash value) which have been already scanned by MobSF.
ls ~/mobsf/Mobile-Security-Framework-MobSF/uploads/ > ~/Desktop/automation/scans_comparison/scanned_samples.txt
...
do
  ...
  sample_md5=`sed -n -e "$num"p ~/Desktop/automation/scans_comparison/samples.txt | awk '{print $1}'`
  sample_path=`sed -n -e "$num"p ~/Desktop/automation/scans_comparison/samples.txt | awk '{print $2}'`
  ...
  if grep -q "${sample_md5}" ~/Desktop/automation/scans_comparison/scanned_samples.txt;
  then
   rm "$sample_path"
   ...
   sleep 2m
   package_sample=$(python3 ~/Desktop/automation/run/dynamic_scan.py "$name_sample" 2>&1)

   # Run script to download Static and Dynamic Report.
   sleep 10m
   python3 ~/Desktop/automation/run/scan_to_pdf.py "$sample_md5" "$package_sample"
  fi
done